            ...
            <main className="container mx-auto px-6 py-8">
                <div className="text-center py-24">
                    ...
                    <Button asChild className="mt-4">
                        <Link href="/ecommerce/tiktok/reports">Kembali ke List</Link>
                    </Button>
                </div>
            </main>
        );
        ...
        <main className="container mx-auto px-6 py-8">
            ...
            <div className="flex items-center space-x-4 mb-8">
                <Button variant="ghost" size="icon" asChild>
                    <Link href="/ecommerce/tiktok/reports">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                        <Card>
                            <CardContent className="pt-6">
                                ...
                                <p className="text-2xl font-bold text-green-600">
                                    {formatCurrency(result.summaryGlobal.totalSettlement)}
                                </p>
                            </CardContent>
                        </Card>
                        <Card>
                        ...
                        <Card>
                            <CardContent className="pt-6">
                                ...
                                <p className="text-2xl font-bold">{result.summaryGlobal.totalMatchedOrders}</p>
                            </CardContent>
                        </Card>
                    </div>
                    ...
                    <Tabs defaultValue="daily" className="space-y-4">
                        <TabsList>
                            ...
                            <TabsTrigger value="unmatched">
                                Unmatched
                                {(result.unmatchedKelola.length > 0 || result.unmatchedIncome.length > 0) && (
                                    <span className="ml-2 px-2 py-0.5 rounded-full bg-yellow-100 text-yellow-700 text-xs">
                                        {result.unmatchedKelola.length + result.unmatchedIncome.length}
                                    </span>
                                )}
                            </TabsTrigger>
                        </TabsList>
                                            ...
                                            {result.summaryDaily.map((day) => (
                                                <TableRow key={day.date}>
                                                    ...
                                                    <TableCell className="text-right text-green-600">
                                                        {formatCurrency(day.totalSettlement)}
                                                    </TableCell>
                        ...
                        <TabsContent value="unmatched">
                            <div className="space-y-4">
                                {result.unmatchedKelola.length > 0 && (
                                    <Card>
                                        <CardHeader>
                                            <CardTitle className="flex items-center gap-2">
                                                <AlertTriangle className="w-5 h-5 text-yellow-500" />
                                                Unmatched dari Kelola ({result.unmatchedKelola.length})
                                            </CardTitle>
                                            <CardDescription>
                                                Order ID yang ada di Kelola tapi tidak ditemukan di Income
                                            </CardDescription>
                                        </CardHeader>
                                        <CardContent className="p-0">
                                            <Table>
                                                ...
                                                <TableBody>
                                                    {result.unmatchedKelola.slice(0, 50).map((item, idx) => (
                                                        <TableRow key={idx}>
                                            ...
                                            {result.unmatchedKelola.length > 50 && (
                                                <p className="text-center py-2 text-sm text-muted-foreground">
                                                    ... dan {result.unmatchedKelola.length - 50} lainnya
                                                </p>
                                            )}
                                        </CardContent>
                                    </Card>
                                ...
                                {result.unmatchedIncome.length > 0 && (
                                    <Card>
                                        <CardHeader>
                                            <CardTitle className="flex items-center gap-2">
                                                <AlertTriangle className="w-5 h-5 text-yellow-500" />
                                                Unmatched dari Income ({result.unmatchedIncome.length})
                                            </CardTitle>
                                            <CardDescription>
                                                Order ID yang ada di Income tapi tidak ditemukan di Kelola
                                            </CardDescription>
                                        </CardHeader>
                                        <CardContent className="p-0">
                                            <Table>
                                                ...
                                                <TableBody>
                                                    {result.unmatchedIncome.slice(0, 50).map((item, idx) => (
                                                        <TableRow key={idx}>
                                                            <TableCell className="font-mono">{item.orderId}</TableCell>
                                                        </TableRow>
                                                    ))}
                                            ...
                                            {result.unmatchedIncome.length > 50 && (
                                                <p className="text-center py-2 text-sm text-muted-foreground">
                                                    ... dan {result.unmatchedIncome.length - 50} lainnya
                                                </p>
                                            )}
                                        </CardContent>
                                    </Card>
